...
module.exports.createRequestTracking = async(req,res)=>{
    //  console.log(req.body);
     try {
        ...
        console.error("Error creating RequestTracking:", err.message);
        res.status(500).json({ message: "Server error", error: err.message });
    }
}

module.exports.getRequestTracking = async(req, res)=> {
    try{
...
module.exports.deleteRequestTracking = async(req, res)=> {
    try{
        const { id } = req.params;
        const deleteRequests = await Request.findByIdAndDelete(id);
        if(!deleteRequests){
            return res.status(404).json({ msg : "RequestTracking data Not Found" });
        }
        res.json({ msg: "RequestTracking deleted succsessfully", deleteRequests });
    }
    catch(err){
...
module.exports.updateRequestTracking = async(req, res) => {
    try {
        const { id } = req.params;
        const { Requester_Name, Request_name, Request_Date, wing, unit, Priority, status } = req.body;
        ...
        if (!updateRequest) {
            return res.status(404).json({ msg: "RequestTracking Not Found" });
        }
        res.json({ msg: "RequestTracking updated successfully", updateRequest });
    } 
    catch(err){
        console.error("Error updating RequestTracking:", err.message);
